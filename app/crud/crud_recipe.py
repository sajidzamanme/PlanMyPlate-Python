...
class CRUDRecipe(CRUDBase[Recipe, RecipeCreateDto, RecipeCreateDto]):
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100) -> List[Recipe]:
        return db.query(self.model).filter(self.model.is_deleted == False).order_by(self.model.name).offset(skip).limit(limit).all()
    ...
    def search_by_name(self, db: Session, name: str) -> List[Recipe]:
        return db.query(Recipe).filter(Recipe.name.ilike(f"%{name}%"), Recipe.is_deleted == False).order_by(Recipe.name).all()

    def filter_by_calories(self, db: Session, min_cals: int, max_cals: int) -> List[Recipe]:
        return db.query(Recipe).filter(Recipe.calories >= min_cals, Recipe.calories <= max_cals, Recipe.is_deleted == False).order_by(Recipe.name).all()
